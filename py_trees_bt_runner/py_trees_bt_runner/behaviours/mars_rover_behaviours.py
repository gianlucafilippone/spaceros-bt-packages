...
class TakePicture(ToBlackboard):
    def __init__(self, name="TakePicture", picture_id="0", save_path="/"):
        super().__init__(topic_name="/camera/image_raw", topic_type=Odometry, qos_profile=10, blackboard_variables={"last_camera_frame": None}, name=name, clearing_policy=py_trees.common.ClearingPolicy.ON_SUCCESS)
        self.picture_id = int(picture_id)
        self.filename = os.path.join(save_path, f"picture_{picture_id}")
    ...
    def update(self):
        ...
        if self.blackboard.exists(self.blackboard_key):
            self.logger.info(f"Picture with ID {self.picture_id} already taken. Taking another picture.")
            self.blackboard.unset(self.blackboard_key)
